...
/*   Updated: 2021/09/13 21:09:22 by pveeta           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	presort(int *arguments, int counter, int flag)
{
	...
	if (flag == 1)
	{
		...
		res = checker_begin(stack_a, counter, 0);
	return (res);
}
...
int	checker_begin(t_list *stack_a, int size_arr, int res)
{
	...
	while (res >= 0)
	{
		res = get_next_line(0, &command);
		if (res > 0)
		{
			select_command(info, command);
			free(command);
		}
		else if (res == 0)
			res = -2;
